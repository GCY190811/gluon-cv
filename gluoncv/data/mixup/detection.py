...
class MixupDetection(Dataset):
    ...
    *args : list
    ...
    def __init__(self, dataset, mixup=None, *args):
        ...
        self._mixup = mixup  #随机数发生器，或者Beta系数np.random.beta
    ...
    def __getitem__(self, idx):  #将对象作为迭代器, 效果类似于__iter__,__next__
        # first image
        ...
        if self._mixup is not None:
            lambd = max(0, min(1, self._mixup(*self._mixup_args))) #?  *arg用法

        if lambd >= 1: #img不变，修改了label1
            weights1 = np.ones((label1.shape[0], 1))
            label1 = np.hstack((label1, weights1)) #每个标签添加一列，代表权重
            return img1, label1

        # second image
        idx2 = np.random.choice(np.delete(np.arange(len(self)), idx)) # 排除img1的index
        img2, label2 = self._dataset[idx2]

        # mixup two images
        height = max(img1.shape[0], img2.shape[0])
        width = max(img1.shape[1], img2.shape[1])
        mix_img = mx.nd.zeros(shape=(height, width, 3), dtype='float32')  #多余区域为0
        ...
        y1 = np.hstack((label1, np.full((label1.shape[0], 1), lambd))) # 每个标签添加1列，权重
        y2 = np.hstack((label2, np.full((label2.shape[0], 1), 1. - lambd)))
        mix_label = np.vstack((y1, y2)) # 竖向叠加
